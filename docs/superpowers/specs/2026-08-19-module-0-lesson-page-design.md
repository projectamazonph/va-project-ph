...
## Errata (applied 2026-08-19)

The Prisma-less migration at `supabase/migrations/202608180001_learning_foundation.sql`
is the source of truth for column shapes. The first iteration of the compile path
and Zod schemas assumed a richer schema than the migration actually defines:

- `modules` has no `slug`, `goal`, or `est_minutes`. Module is upserted by
  `(course_id, position)`. The URL keeps the `module-N` convention and a small
  Zod transformer (`ModuleRoutingSlugSchema`) maps it to a position.
- `lessons.content` is `jsonb` (the body is wrapped as `{ format: "mdx", raw: <string> }`),
  not a `body` text column. `is_published: true` is set at compile time.
- A `courses` row is upserted in `compile-mdx.ts main()` before the module loop,
  using `module.courseSlug` from `_meta.json`. The course slug for Module 0 is
  `amazon-ppc-foundations`.

PRs #2–#5 reference `module.goal` and `lesson.body` in their plan snippets; the
implementer must follow the migration, not the spec snippets. Module-level
`est_minutes` is not displayed; lesson-level `estimatedMinutes` is.
